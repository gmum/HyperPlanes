...
class ShapeNet128x128(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"images": all_imgs, "poses": all_poses, "hwf": hwf, "bound": bound, "focal": focal}
...
synth_id_to_category = {
    "02691156": "planes",
    "02773838": "bag",
    "02801938": "basket",  # airplane = planes temporary
    "02808440": "bathtub",
    "02818832": "bed",
    "02828884": "bench",
    "02834778": "bicycle",
    ...
    "02954340": "cap",
    "02958343": "cars",
    "03001627": "chairs",  # car=cars temporary chair=chairs
    "03046257": "clock",
    "03207941": "dishwasher",
    "03211117": "monitor",
    "04379243": "table",
...
class ShapeNet200x200(Dataset):
    def __init__(
        self,
        root_dir="/home/datasets/nerfdataset",
        shapenet_root_dir="/shared/sets/datasets/3D_points/ShapeNetCore.v2",
        classes=[],
        train=True,
    ):
        """
        ...
        self.hwf = torch.as_tensor([200, 200, self.focal], dtype=torch.float)

        # all shapenet scenes are bounded between 2. and 6.
        near = 2.0
        far = 6.0
        self.bound = torch.as_tensor([near, far], dtype=torch.float)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "poses": sample["cam_poses"].astype(np.float32),
            "hwf": self.hwf,
            "bound": self.bound,
        }
    ...
    def _load(self):
        ...
        self.train_data = pd.DataFrame(
            columns=["class", "name", "sample_filename", "obj_filename"]
        )
                            ...
                            [
                                {
                                    ...
                                    "obj_filename": join(
                                        self.shapenet_root_dir,
                                        category_to_synth_id[data_class],
                                        sample_name,
                                        "models",
